...
// esbuild lives in .ds-sync/node_modules (not the repo root), so resolve it
// via a relative path from this override's location in .design-sync/overrides/.
...
// Stub plugin: intercepts large game-data JSON files so they don't inflate
// the design-sync IIFE (these files live in public/data/ and _references/,
// weigh several MB each, and contain runtime D&D catalog data that visual
// previews don't need). Components render their UI structure with empty
// arrays/objects; the actual data loads at runtime from the Next.js server.
const dataSizeStubPlugin = {
  name: 'data-size-stub',
  setup(b) {
    // Stub public/data JSON files — imported by ruleService, itemCatalogService, etc.
    b.onResolve({ filter: /\/public\/data\/.*\.json$/ }, (a) => ({ path: a.path, namespace: 'ds-data-stub' }));
    // Stub _references JSON files — foundry/personagem reference data.
    b.onResolve({ filter: /\/_references\/.*\.json$/ }, (a) => ({ path: a.path, namespace: 'ds-data-stub' }));
    // Deep-proxy stub: any property access returns another array-proxy so
    // chains like `data.background.filter(...)` never throw.
    b.onLoad({ filter: /.*/, namespace: 'ds-data-stub' }, () => ({
      contents: `
const _mk = () => new Proxy([], {
  get(t, p) {
...
function sharedBuildOptions({ nodePaths, tsconfig }) {
  ...
  const plugins = [dataSizeStubPlugin, reactShim];
